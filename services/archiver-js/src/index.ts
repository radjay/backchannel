...
type MonitoredRoom = { room_id: string; enabled: boolean; archive_media?: boolean; auto_join?: boolean };
...
class Archiver {
  ...
  private managedRooms = new Set<string>();
  private lastRoomRefresh = 0;
  ...
  private async matrixLogin(): Promise<void> {
    ...
    while (!this.matrixToken) {
      ...
      if (resp.status === 429) {
        ...
        const waitMs = Number((body as Record<string, unknown>).retry_after_ms ?? 60000);
  ...
  private async resolveRoomName(roomId: string): Promise<{ room_name: string; room_display_name: string }> {
    if (!this.matrixToken) return { room_name: '', room_display_name: roomId };
    ...
    try {
      const nameResp = await fetch(
        `${MATRIX_HOMESERVER_URL}/_matrix/client/r0/rooms/${encodeURIComponent(roomId)}/state/m.room.name?${authParams}`
      );
      if (nameResp.ok) {
        const data = (await nameResp.json()) as Record<string, unknown>;
        roomName = typeof data?.name === 'string' ? (data.name as string) : '';
      }
    } catch (err) {
    ...
    try {
      const aliasResp = await fetch(
        `${MATRIX_HOMESERVER_URL}/_matrix/client/r0/rooms/${encodeURIComponent(
          roomId
        )}/state/m.room.canonical_alias?${authParams}`
      );
      if (aliasResp.ok) {
        const data = (await aliasResp.json()) as Record<string, unknown>;
        displayName = roomName || (typeof data?.alias === 'string' ? (data.alias as string) : '');
      }
    } catch (err) {
  ...
  private async resolveUserDisplayName(userId: string, roomId: string): Promise<string> {
    if (!this.matrixToken) return userId;
    const authParams = new URLSearchParams({ access_token: this.matrixToken });
    ...
    try {
      const memberResp = await fetch(
        `${MATRIX_HOMESERVER_URL}/_matrix/client/r0/rooms/${encodeURIComponent(
          roomId
        )}/state/m.room.member/${encodeURIComponent(userId)}?${authParams}`
      );
      if (memberResp.ok) {
        const data = (await memberResp.json()) as Record<string, unknown>;
        if (typeof data?.displayname === 'string') return data.displayname as string;
      }
    } catch (err) {
    ...
    try {
      const profileResp = await fetch(
        `${MATRIX_HOMESERVER_URL}/_matrix/client/r0/profile/${encodeURIComponent(userId)}/displayname?${authParams}`
      );
      if (profileResp.ok) {
        const data = (await profileResp.json()) as Record<string, unknown>;
        if (typeof data?.displayname === 'string') return data.displayname as string;
      }
    } catch (err) {
  ...
  private isNoticeOrError(event: MatrixMessageEvent): boolean {
  ...
  private determineNoticeType(event: MatrixMessageEvent): string {
    ...
    if (body.includes('warning') || (event.content?.body ?? '').includes('⚠️')) return 'warning';
    return 'info';
  }

  private async archiveNotice(event: MatrixMessageEvent): Promise<void> {
  ...
  private async archiveMessage(event: MatrixMessageEvent): Promise<void> {
  ...
  private async archiveMediaIfPresent(event: MatrixMessageEvent): Promise<void> {
  ...
  private async downloadMatrixMedia(mxcUrl: string): Promise<Buffer | null> {
    if (!this.matrixToken) return null;
  ...
  private async saveMediaMetadata(event: MatrixMessageEvent, storagePath: string | null, content: MatrixMessageEvent['content']) {
  ...
  private async acceptRoomInvitation(roomId: string): Promise<void> {
    if (!this.matrixToken) return;
  ...
  private async refreshRoomManagement(force = false): Promise<void> {
    const now = Date.now() / 1000;
    if (!force && now - this.lastRoomRefresh < ROOM_REFRESH_INTERVAL_SECONDS) return;
    ...
    const enabledRooms = new Set((data ?? []).filter((r: MonitoredRoom) => r.enabled !== false).map((r) => r.room_id));
  ...
  private async joinRoomIfPossible(roomId: string): Promise<void> {
    if (!this.matrixToken) return;
    const url = `${MATRIX_HOMESERVER_URL}/_matrix/client/r0/rooms/${encodeURIComponent(roomId)}/join?access_token=${this.matrixToken}`;
    try {
      const resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
  ...
  private async syncLoop(): Promise<void> {
    ...
    while (true) {
      try {
        ...
        const syncData = (await resp.json()) as Record<string, any>;
        ...
        for (const [roomId, roomData] of Object.entries(joinedRooms)) {
          if (!this.managedRooms.has(roomId)) continue;
          const roomDataAny = roomData as Record<string, any>;
          const events: MatrixMessageEvent[] = roomDataAny?.timeline?.events ?? [];
